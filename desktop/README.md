...
The `predev` and `prebuild` scripts copy the existing mobile `skeleton_pre-cut.glb` into the desktop local assets. This asset is cached with the offline shell. It retains the mobile asset attribution: Skeleton Pre-cut · Maxime66410 · Sketchfab Standard. GLB files remain excluded from Git, consistent with the mobile asset instructions. The desktop no longer uses procedurally generated stick/guide skeletons.
