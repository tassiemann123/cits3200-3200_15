    ...
    <div className="application">
      <main className="workbench">
        ...
        <section className="viewer-panel" aria-label="Skeleton analysis workspace">
          ...
          {mainTab === 'scene' ? <>
            <div className="scene-toolbar"><div className="view-toggle">{(['perspective', 'front', 'top'] as const).map(v => <button key={v} className={view === v ? 'active' : ''} onClick={() => setView(v)}>{v === 'perspective' ? '3D view' : v[0].toUpperCase() + v.slice(1)}</button>)}</div><div className="scene-options"><button className={showGrid ? 'active' : ''} aria-label="Toggle grid" aria-pressed={showGrid} title="Grid" onClick={() => setShowGrid(v => !v)}><Grid2X2 size={16} /></button><button className={showMarkers ? 'active' : ''} aria-label="Toggle recorded joint markers" aria-pressed={showMarkers} title="Recorded joint markers" onClick={() => setShowMarkers(v => !v)}><MapPin size={16} /></button><span /><button aria-label="Fit all skeletons" title="Fit all" onClick={() => { setFrameKey(n => n + 1); setZoom(1); }}><Maximize size={16} /></button></div></div>
            <div className="scene-area">
              <SceneViewport individuals={project.individuals} selectedId={selected.id} selectedJointId={joint?.id ?? ''} onSelect={(id, pointId) => { setSelectedId(id); setJointId(pointId); setInspectorTab('coordinates'); }} showGrid={showGrid} showMarkers={showMarkers} view={view} frameKey={frameKey} zoom={zoom * 100} />
